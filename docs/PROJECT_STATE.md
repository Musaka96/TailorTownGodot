...
| **WorldScale** | `globals/world_scale.gd` | The proportions dial: scales the player model, every other character rig, the movable props/stations and the camera's home offset by one factor, live. Ships at 0.85 (a 1.90 m character); the F3 slider moves it in debug builds. See the note below the table. |
...
**World scale.** The characters, the movable props and the camera distance are scaled
together by `WorldScale` at runtime; the building, floors and anything in its `SKIP_NAMES`
list stay at their authored real-metre size, and collision capsules, reach, walk speed and
nav are never touched. The shipped value is 0.85 (`WorldScale.DEFAULT`), chosen on
2026-09-24 after the Yardstick proportions review: the 2.24 m character becomes 1.90 m
against the 2.4 m door and 3 m walls. In debug builds the F3 panel's World section has a
Scale slider (0.70 to 1.00) with a height readout and a Reset (back to 0.85); that value
survives restarts in `user://debug_settings.cfg` under `[debug] world_scale`, separate
from the save game. 1.00 shows the world as authored.
