...
func _drop() -> PackedFloat32Array:
	...
	_links(out, 0.0, DOWN * 0.9, 16, 1.4, 0.5, 1500.0)
...
func _hoist() -> PackedFloat32Array:
	...
	_links(out, 0.03, UP + 0.1, 14, 0.8, 0.45, 1900.0)
	return out
...
func _links(
	...
	for _k in count:
		...
		_link(out, at, pitch * _rng.randf_range(0.8, 1.35), gain * _rng.randf_range(0.3, 0.7))
...
## The board settling against its chains: a woody knock with a little hollow body.
func _knock(out: PackedFloat32Array, at: float, level: float) -> void:
	...
	for i in int(0.35 * RATE):
		...
		out[idx] += (body * 0.4 + slap * exp(-t * 40.0) * 0.8) * level


## Wood and iron taking the weight: a gritty tone sliding from `f0` to `f1`.
func _creak(out: PackedFloat32Array, at: float, length: float, f0: float, f1: float) -> void:
	...
	for i in int(length * RATE):
		...
		# A sawtooth-ish rasp, gated by slow stick-slip flutter so it grinds, not hums.
		var rasp := fmod(phase, TAU) / PI - 1.0
		var grip := 0.5 + 0.5 * sin(TAU * 23.0 * p * length + sin(p * 9.0) * 2.0)
