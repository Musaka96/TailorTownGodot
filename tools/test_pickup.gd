...
const WAITING := 4  # Customer.Mode.WAITING
...
func _apologies() -> void:
	...
	_check(_orders.apologise(order) == "moved", "a stranger can be moved to tomorrow")
	...
	# ...but only once.
	before = _rep.points
	_check(_orders.apologise(order) == "lost", "no second chance on a late order")
	...
	_check(_orders.apologise(order) == "lost", "a stranger may refuse")
	...
	before = _rep.points
	_check(_orders.apologise(order) == "regular", "a regular always agrees")
	...
	_check(not _orders.can_reschedule(order), "an event suit can't move past the event")
	_check(_orders.apologise(order) == "event", "event: the order is lost")
...
func _left_standing() -> void:
	...
	_check(cust != null and cust.get("_mode") == WAITING, "not ready: he waits at the counter")
...
func _finished_in_time() -> void:
	...
	var cust := await _send_collector(order)
	_check(cust != null and cust.patience() > 0.9, "a fresh wait starts full")
...
func _send_collector(order: Resource) -> Node:
	...
	for _i in 300:
		...
		for cust in get_nodes_in_group("customer"):
			if cust.collect_order == order and int(cust.get("_mode")) != 0:
				return cust
	return null
